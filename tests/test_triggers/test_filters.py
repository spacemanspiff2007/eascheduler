...
def test_dow_filter():
    ...
    assert not f.allow(DateTime(2001, 1, 1))

    for i in range(2, 7):
        assert f.allow(DateTime(2001, 1, i))

    assert not f.allow(DateTime(2001, 1, 8))
...
def test_dom_filter():
    ...
    assert not f.allow(DateTime(2001, 2, 1))


def test_moy_filter():
    f = MonthOfYearProducerFilter([1, 3, 4])
    assert f.allow(DateTime(2001, 1, 31))

    for i in range(1, 29):
        assert not f.allow(DateTime(2001, 2, i))

    assert f.allow(DateTime(2001, 3, 1))


def test_inverting_filter():
    f = InvertingProducerFilter(DayOfWeekProducerFilter([2, 3, 4, 5, 6, 7]))
    assert f.allow(DateTime(2001, 1, 1))

    for i in range(2, 7):
        assert not f.allow(DateTime(2001, 1, i))

    assert f.allow(DateTime(2001, 1, 8))


def test_or_group():
    f = AnyGroupProducerFilter([DayOfWeekProducerFilter([3, 4, 5, 6, 7]), DayOfWeekProducerFilter([1, 4, 5, 6, 7])])
    assert f.allow(DateTime(2001, 1, 1))
    assert not f.allow(DateTime(2001, 1, 2))
    assert f.allow(DateTime(2001, 1, 3))
    assert f.allow(DateTime(2001, 1, 4))
...
def test_and_group():
    ...
    assert not f.allow(DateTime(2001, 1, 1))
    assert not f.allow(DateTime(2001, 1, 2))
    assert not f.allow(DateTime(2001, 1, 3))
    assert f.allow(DateTime(2001, 1, 4))
