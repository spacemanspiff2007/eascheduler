...
async def test_eq():
    job1 = OneTimeJob(SyncExecutor(lambda: 1/0), DateTime.now(tz=local_tz).add(seconds=0.01))
    job2 = OneTimeJob(SyncExecutor(lambda: 1/0), DateTime.now(tz=local_tz).add(seconds=0.01))
...
async def test_onetime():
    ...
    s = AsyncScheduler()
    job = OneTimeJob(SyncExecutor(lambda: 1/0), DateTime.now(tz=local_tz).add(seconds=0.01))
...
async def test_base_properties():
    ...
    target = DateTime.now(tz=local_tz).add(seconds=0.01)
    s = AsyncScheduler()
