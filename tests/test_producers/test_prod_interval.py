...
from tests.helper import cmp_utc_with_german, get_ger_str, get_german_as_instant, get_local_as_instant


def test_simple():
    dt_start = get_local_as_instant(1, 1, 1)
    dt_now = dt_start

    p = IntervalProducer(get_local_as_instant(1, 1, 8), 3600 * 5)
    ...
    for v in range(3, 100, 5):
        ...
        dt_now = get_local_as_instant(1, dt_start.day + days, hours)


def test_filter():
    producer = IntervalProducer(get_local_as_instant(1, 1, 8), 3600 * 12)

    for _ in range(10):
        assert producer.get_next(get_local_as_instant(1, 1, 7)) == get_local_as_instant(1, 1, 8)
        assert producer.get_next(get_local_as_instant(1, 1, 8)) == get_local_as_instant(1, 1, 20)
        assert producer.get_next(get_local_as_instant(1, 1, 20)) == get_local_as_instant(1, 2, 8)

    producer._filter = DayOfWeekProducerFilter([6])
    for _ in range(10):
        assert producer.get_next(get_local_as_instant(1, 1, 7)) == get_local_as_instant(1, 6, 8)
        assert producer.get_next(get_local_as_instant(1, 6, 8)) == get_local_as_instant(1, 6, 20)
        assert producer.get_next(get_local_as_instant(1, 6, 20)) == get_local_as_instant(1, 13, 8)
